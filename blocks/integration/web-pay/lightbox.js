...
export const WebPayLightbox = (props) => {
    ...
    const [clientSecret, setClientSecret] = useState(settings.lightbox.client_secret);
    ...
    useEffect(() => {
        ...
        monriRef.current = Monri(settings.lightbox.authenticity_token, {
            locale: settings.lightbox.locale
        });

        const components = monriRef.current.lightbox({clientSecret: clientSecret});
